...
Test("same-count IDs changing during cache lookup invalidate capture",()=>{Accept();StyxWoW.Cache.AfterLookup=_=>ObjectManager.Wow!.Slots[0].Id=999;var s=Capture();Check(!Complete(s)&&Ids(s).Contains(867),"replacement hidden by count or identity discarded");});
...
Test("cache read exception preserves raw identities",()=>{Accept();StyxWoW.Cache.Failure=new InvalidOperationException("cache unavailable");var s=Capture();Check(!Complete(s)&&Ids(s).Contains(867),"cache failure discarded observed occupied identity");});
...
Test("cache hydration timeout does not invent completion",()=>{Accept(hydrate:false);StyxWoW.Cache.Failure=new TimeoutException();var s=Capture();Check(!Complete(s)&&Ids(s).Contains(867),"timeout erased identity");});
