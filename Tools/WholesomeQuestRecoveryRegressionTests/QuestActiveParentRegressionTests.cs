...
internal static class QuestActiveParentRegressionTests
{
    ...
    private static void Expect(int parent,uint[] accepted,uint[] rewarded,bool expected,bool metadata,bool ready=false,int[]? required=null,bool authority=true,bool completeLog=true)
    {
        var r=Plan(parent,accepted,rewarded,metadata,ready,required,authority,completeLog);
        Check(Pickup(r,200)==expected,$"expected pickup={expected}, observed {Pickup(r,200)}; parent={parent}");
    }
    private static QuestScheduleResult Plan(int parent,uint[] accepted,uint[] rewarded,bool metadata,bool ready=false,int[]? required=null,bool authority=true,bool completeLog=true,bool unrelated=false)
    {
        ...
        var snapshot=new QuestSchedulerSnapshot{UtcNow=new DateTime(2026,9,18,0,0,0,DateTimeKind.Utc),PlayerLevel=20,PlayerRaceId=1,MapId=1,HasCompleteQuestLog=completeLog,HasAuthoritativeCompletions=authority,CompletedQuestIds=rewarded,AcceptedQuests=accepted.Select(id=>new QuestSchedulerAcceptedQuest{QuestId=id,IsCompleted=ready&&id==100}).ToArray()};
