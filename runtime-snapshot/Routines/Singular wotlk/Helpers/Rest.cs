        ...
        public static Composite CreateDefaultRestBehaviour()
        {
            return
                ...
                // Check if we're allowed to eat (and make sure we have some food. Don't bother going further if we have none.
                        new Decorator(
                            ret =>
                            RestConsumablePolicy.CanUseAutomaticConsumables(StyxWoW.Me.CurrentMap.IsInstance) &&
                            RestConsumablePolicy.CanEat(CharacterSettings.Instance.FoodAmount) &&
                            !LiquidEnvironment.IsPlayerInLiquid(StyxWoW.Me) && StyxWoW.Me.HealthPercent <= SingularSettings.Instance.MinHealth && !StyxWoW.Me.HasAura("Food") &&
                ...
                // Make sure we're a class with mana, if not, just ignore drinking all together! Other than that... same for food.
                        new Decorator(
                            ...
                            !LiquidEnvironment.IsPlayerInLiquid(StyxWoW.Me) && (StyxWoW.Me.PowerType == WoWPowerType.Mana || StyxWoW.Me.Class == WoWClass.Druid) &&
